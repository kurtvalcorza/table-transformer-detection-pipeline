"""Static release-asset validation for the Table Transformer (DETR-R18) table-detection DIMER pipeline.
...
EXPECTED_MODEL_ID = "microsoft/table-transformer-detection"
PIPELINE_CLASS = "TableTransformerDetectionPipeline"
MODEL_LOAD_EXPR = f"{PIPELINE_CLASS}.from_pretrained(weights_dir=WEIGHTS_DIR)"
KNOWN_SHAS: frozenset[str] = frozenset()  # the 121 photograph digests live in the carried samples module, not in prose
BYOD_GATES = ("USE_BYOD",)
EXPECTED_OUTPUTS = (
    "outputs/table_transformer_detection_train.csv",
    "outputs/table_transformer_detection_input_manifest.json",
    "outputs/table_transformer_detection_evaluation_report.json",
    "outputs/table_transformer_detection_preview.png",
    "outputs/table_transformer_detection_adapter",
    "outputs/table_transformer_detection_result.json",
)
CODE_MARKERS = (
    # Stage 4: pinned corpus, validation, product-level split, CSV, refusal probes
    "USE_BYOD = False",
    ...
    # Stage 5: the inference contract with its manifest, rejection probe, sanity checks and sample-sanity report
    ...
    "report = evaluation_report(low, probe_record['boxes'], sample_kind=",
    "assert report['verdict'] == 'sample-sanity'",
    # Stage 6: the prior, the zero-shot checkpoint and the frozen policy
    ...
    # Stage 9: detections before/after, preview, artifact, reload parity, provenance
...
MARKDOWN_MARKERS = (
    "**Capability:** table detection on document page images (boxes labelled `table` or `table rotated`) and bounded supervised adaptation",
    "new detection heads on the frozen DETR decoder features with an optional unfreeze of the last decoder layers",
    "**supervised adaptation to a new box class on a new image domain, under an explicit frozen-vs-unfrozen policy**",
    "**fixed-box prior**",
    ...
    "**AP@0.5**",
    "**`sample-sanity`**",
    "The PubTables heads and `detect` are never trained or exported",
...
EXTERNAL_WEIGHT_BYTES: dict[int, str] = {
    129_318_620: "total of the pinned Open Food Facts nutrition-table images (dataset)",
}
