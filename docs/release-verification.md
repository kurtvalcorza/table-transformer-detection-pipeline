...
`tutorials/table_transformer_detection_colab.ipynb` (`TASK-INFERENCE`, **standalone** carrier) is a
...
- the standalone carrier (ST1–ST6, PAR1–PAR3): no clone, repository install or repository import on
  ...
  `src/table_transformer_detection_pipeline/pipeline.py` after the generator's documented rewrites; the
...
- the profile-specific public-API calls (`stage_missing_files`, `verify_snapshot`,
  `TableTransformerDetectionPipeline.from_pretrained(weights_dir=...)`, `validate_inputs`, `detect`,
  `evaluation_report`), the ceiling print (`MIN_IMAGE_SIDE`, `MAX_IMAGE_SIDE`, `MAX_DETECTIONS`,
  `LABELS`, `DETECTION_THRESHOLD`), the exports, the learner-facing statements (caller-owned
...
3. run the notebook top-to-bottom without editing implementation cells (form parameters at their
   defaults for the sample path: `USE_BYOD = False`, `threshold = 0.9`);
...
5. verify every default-path stage completes:
   ...
   - the carried module cell executes (defines `TableTransformerDetectionPipeline`, `validate_inputs`,
     `evaluation_report`, `box_iou`, `verify_snapshot`, `stage_missing_files`) with no import of the
     repository package;
   - synthetic 850×1100 page rendered in code with its RGB SHA-256 printed and the ceilings
     (`MIN_IMAGE_SIDE` 16, `MAX_IMAGE_SIDE` 4096, `MAX_DETECTIONS` 15, the two `LABELS`,
     `DETECTION_THRESHOLD` 0.9) surfaced;
   - pinned `microsoft/table-transformer-detection` acquisition at the immutable revision through the
     carried module: the inline `MANIFEST` is asserted against the module identity and written to
     `weights/table-transformer-detection/`, `stage_missing_files(WEIGHTS_DIR, allow_download=True)` reports
   ...
   - `validate_inputs` writes `outputs/table_transformer_detection_input_manifest.json` (verdict
   ...
   - `detect` returning score-ordered detections; record the labels, scores and boxes (the card-pass
     CPU smoke returned two `table` boxes at 0.9985 and 0.9973, inset from the drawn rules with
     `box_iou` 0.80 / 0.66; a materially different result is a finding to record, not a failure by itself, because no metric
     is asserted);
   - `evaluation_report` writes `outputs/table_transformer_detection_evaluation_report.json` with verdict
     `sample-sanity` and one `box_iou` entry per drawn reference box on the synthetic sample
     (`not-measurable` on BYOD), stated as such;
   - `outputs/table_transformer_detection_result.json`, `outputs/table_transformer_detection_detections.csv`
     and `outputs/table_transformer_detection_annotated.png` written with `NOTEBOOK_SOURCE`, model
...
|---|---|---|---|---|---|
| 2026-09-13 | working tree of the initial build — generator output identical to blob `410df1003fef` (commit `1cd378a`) except the `repository_revision` label in `NOTEBOOK_SOURCE` | Local Windows-venv harness (`run_nb_local.py`: nbclient 0.11.0, fresh `python3` kernel, `CUDA_VISIBLE_DEVICES=-1`, `DIMER_NOTEBOOK_CI_PREINSTALLED=1`), Python 3.12, torch 2.14.0+cu130, transformers 4.57.6, timm 1.0.29 | Default synthetic path, all 8 code cells: pinned install skipped (pre-installed), `stage_missing_files` fetched the 115 MB snapshot from the Hub at the pinned revision into the scratch `weights/`, `verify_snapshot` PASS, `detect` → 2 `table` boxes (0.9985 / 0.9973), `evaluation_report` `sample-sanity` with `box_iou` 0.80 / 0.66, 5 outputs written | 19.9 s | PASS — pre-flight only; not promotion evidence |

### Manual clean-runtime evidence

| Date (UTC) | Commit / notebook blob | Executor | Path exercised | Wall | Outcome |
|---|---|---|---|---|---|
| 2026-09-14 | `0a86159` / `7a62e8eff1a9` | Kaggle CPU (`kurtvalcorza/dimer-nb2-table-transformer-detection` v1) | Default sample path | 228.7 s | **PASSED** — 8/8 ok code cells executed cleanly, 10 files, 115 MB staged |
...
path has not been executed; the model's boxes on the synthetic page are inset relative to the drawn
rules (they exclude the last column and bottom row), which is why the `box_iou` values are 0.80 / 0.66
rather than ~1.0 — a rendering/annotation-convention effect, not a defect the pipeline can detect.